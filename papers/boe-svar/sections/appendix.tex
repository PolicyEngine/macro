...
Table~\ref{tab:data} documents every series in the dataset: source, series codes, and the construction applied. The dataset described here is the paper-vintage dataset, covering 1992Q1--2024Q2 (130 quarters, no missing values); the current data pipeline extends the same construction through 2026Q1. The binding start is the FRED Brent series \texttt{POILBREUSDQ}, which begins in 1992Q1. All variables enter the model as $100\cdot\log$ of the level except Bank Rate, which enters as the level in per cent. The full download-and-construction script ships with the repository (\texttt{scripts/download\_data.py}); raw source files are cached so the build is reproducible offline.
...
\midrule
Production replication & 10{,}000 & 744 & 7.4\% & yes & 356.1 \\
Forecast revision (2024Q2 edge) & 6{,}000 & 487 & 8.1\% & yes & 207.5 \\
Paper figures, estimation sample & 3{,}000 & 241 & 8.0\% & no & --- \\
