...
fig_fevd stacks the posterior MEAN of the per-draw shares of TOTAL
forecast-error variance. It previously stacked pointwise per-shock medians
divided by their sum; those medians add to roughly 0.6-0.7, so renormalising
inflated every identified share by about a third. The x axis is quarters
ahead: column h of the FEVD holds the (h+1)-step variance, so the one-year
reading is 4 quarters, not column 4.
...
Usage: BOE_VAR_MODEL=~/boe-var-model python make_figures.py
...
# Resolvable on any machine. This hardcoded one developer's home directory, so
# nobody else could regenerate these figures — which is how a superseded FEVD
# presentation stayed in the committed PDF.
...
# Frozen paper vintage: estimate through EST_END, condition through EDGE.
EDGE = pd.Period("2024Q2", "Q")
EST_END = pd.Period("2023Q2", "Q")
...
ALL_FIGURES = ("irf", "fevd", "hd", "fan")


def main(wanted=ALL_FIGURES):
    wanted = set(wanted)
    ...
    if df_full.index[-1] < EDGE:
        raise SystemExit(
            f"packaged data ends at {df_full.index[-1]}, before the frozen "
            f"{EDGE} edge these figures are defined on."
        )
    ...
    irf_b = analysis.irf_bands(pairs, horizons=HORIZONS) if "irf" in wanted else None
    # Posterior MEAN of the per-draw shares. Shares sum to one across shocks on
    # every draw, so the mean does too and nothing is renormalised. The figure
    # previously stacked pointwise per-shock MEDIANS and divided by their sum:
    # those medians summed to ~0.6-0.7, so renormalising inflated every
    # identified share by roughly a third. It is also the statistic the source
    # paper's Figure 4 plots ("the mean of the sum of the decomposition").
    fevd_mean = (
        np.stack([analysis.fevd(d, B, HORIZONS) for d, B in pairs]).mean(axis=0)
        if "fevd" in wanted else None)
    ...
    if "fevd" in wanted:
        # Column h of the FEVD holds the (h+1)-step-ahead variance, so the x axis
        # is quarters ahead 1..HORIZONS and the one-year reading is at 4, not 5.
        xq = np.arange(1, HORIZONS + 1)
        fig, axes = plt.subplots(1, 2, figsize=(8.6, 3.1), sharey=True)
        for ax, i, title in ((axes[0], I_GDP, "UK GDP"), (axes[1], I_CPI, "UK CPI")):
            series = [fevd_mean[i, j] for _, j in groups]
            series.append(fevd_mean[i, 3] + fevd_mean[i, 7])  # unidentified
            labels = [g for g, _ in groups] + ["Unidentified"]
            colors = [SHOCK_COLORS[lab] for lab in labels]
            ax.stackplot(xq, series, labels=labels, colors=colors,
                         edgecolor="white", linewidth=0.4)
            ax.axvline(4, color="#333333", lw=0.8, ls=":")
            ax.set_xlim(1, HORIZONS)
            ax.set_ylim(0, 1)
            ax.set_title(title)
        ...
        plt.close(fig)
        iy = analysis.fevd_horizon_index(4)
        print("fig_fevd 1-yr global share (posterior mean, share of total): "
              f"GDP {100 * fevd_mean[I_GDP, :3, iy].sum():.1f}%, "
              f"CPI {100 * fevd_mean[I_CPI, :3, iy].sum():.1f}%")
    ...
    # figure_numbers.json is written only when this run executes: it is that
    # run's output, and other pages read it.
    if "fan" in wanted:
        ...
        out.update({
            ...
            "seed": SEED,
            "fevd_statistic": (
                "Posterior mean of the per-draw share of TOTAL 4-quarter-ahead "
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        help=("comma-separated subset of " + ",".join(ALL_FIGURES) +
              "; regenerating a single figure avoids disturbing artifacts "
              "that other pages read (figure_numbers.json comes from 'fan')"))
    names = [n.strip() for n in ap.parse_args().figures.split(",") if n.strip()]
    unknown = set(names) - set(ALL_FIGURES)
    if unknown:
