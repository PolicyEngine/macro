...
# Resolvable on any machine. The script previously hardcoded one developer's
# home directory, so the figures it produces could not be regenerated by
# anyone else — which is how a bad experiment stayed committed.
REPO = Path(
    os.environ.get("US_FRB_MODEL", Path.home() / "us-frb-model")
).expanduser()
if not (REPO / "vendor").is_dir():
    raise SystemExit(
        f"us-frb-model checkout not found at {REPO}. "
        "Set US_FRB_MODEL to the checkout path."
    )
...
# The peg switches must be in the data BEFORE init_trac, and the deviation
# must be taken against a baseline that was itself solved under the peg.
#
# Flipping dmpintay->0 / dmpex->1 *after* tracking leaves the wedge between
# the inertial-Taylor rule's own value and the actual funds rate sitting in
# the rff block's add-factor, so it keeps being subtracted. The funds rate
# then settles 0.07-0.12pp BELOW its own rfffix target — an unintended
# monetary easing stacked on the fiscal stimulus, which inflated the year-2
# multiplier from 0.90 to 0.99. A pegged run whose rate moves is not pegged,
# and the assertion below is the cheap check that says so.
fixdata = data.copy()
fixdata.loc[START:END40, "dmpintay"] = 0
fixdata.loc[START:END40, "dmpex"] = 1
fixdata.loc[START:END40, "rfffix"] = fixdata.loc[START:END40, "rff"]
base_fix = gmodel.init_trac(START, END40, fixdata)

gdata_fix = base_fix.copy()
gdata_fix.loc[START:FISC_END, "egfe"] = (
    base_fix.loc[START:FISC_END, "egfe"]
    + 0.01 * base_fix.loc[START:FISC_END, "xgdp"]
)
sim_g_fix = gmodel.solve(START, END40, gdata_fix)
drift = (sim_g_fix.loc[START:END40, "rff"]
         - base_fix.loc[START:END40, "rff"]).abs().max()
assert drift < 1e-10, f"funds rate is not pegged: max |drift| = {drift:.3e}"
results["gov_fixed"] = dev(sim_g_fix, base_fix)
