...
\paragraph{No shock uncertainty.} The reported responses are point paths, not distributions. Much of the published FRB/US policy literature --- \citet{reifschneider2000three}, \citet{reifschneider2016gauging}, \citet{bernanke2019strategies} --- rests on \emph{stochastic} simulations in which historical residuals are drawn repeatedly and statistics such as the frequency of binding lower bounds are computed across thousands of paths. The package now provides a stochastic-simulation driver --- \texttt{stochsim\_bands()}, a seeded joint historical-residual bootstrap using the official \texttt{stochastic\_type} inventory, preserving contemporaneous residual dependence and keeping failed replications visible --- so questions of that type (how often does the ZLB bind? what is the variance of inflation under rule X?) are in principle answerable. The public hosted runs remain deterministic, all experiments in this paper are deterministic, and no probability result is claimed without a separately reviewed residual window, policy closure, and convergence report (Section~\ref{sec:limitations}); the deterministic responses reported here are the impulse-response building blocks such exercises use.
...
\begin{enumerate}
  ...
  \item \textbf{Validation gates.} \texttt{pytest} runs the cross-validation test against the stored \texttt{pyfrbus} reference (all 284 endogenous variables, 20 quarters, gate $<10^{-6}$; observed $6.0\times10^{-9}$) --- the gate that carries the fidelity claim --- and the tracking-invariant test (gate $<10^{-8}$; observed $5.6\times10^{-17}$), which protects the add-factor machinery but is an identity and is not evidence about the model (Section~\ref{subsec:tracking}); the companion test \texttt{test\_tracking\_invariant\_\allowbreak holds\_for\_arbitrary\_data} pins that by passing the same gate on randomly rescaled data. \texttt{scripts/generate\_vendor\_reference.sh} documents how the vendor reference was produced in a separate patched environment.
  \item \textbf{Experiments.} \texttt{figures/run\_experiments.py} (alongside this paper's source) runs all six experiments of Section~\ref{sec:results} --- the 40-quarter monetary shock, the purchases shock under both monetary regimes, the tax cut and the cost-push shock --- writing \texttt{results.json} with every reported series and the colourblind-safe PDF figures; \texttt{figures/make\_comparison\_figures.py} draws the comparison overlays of Section~\ref{sec:comparison}. Total runtime is a few seconds; the benchmark numbers of Table~\ref{tab:perf} are printed by the same script.
