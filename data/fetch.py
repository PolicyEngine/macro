#!/usr/bin/env python3
"""Fetch UK and US official statistics into an append-only vintage store.
...
FRED_SERIES = {
    "us_real_gdp": {
        "cdid": "GDPC1",
        "title": "US real gross domestic product",
        "frequency": "quarterly",
        "units": "billions of chained 2017 dollars, seasonally adjusted annual rate",
    },
    "us_cpi": {
        "cdid": "CPIAUCSL",
        "title": "US consumer price index, all urban consumers",
        "frequency": "monthly",
    ...
    "us_treasury_10y": {
        "cdid": "DGS10",
        "title": "10-year US Treasury constant maturity rate",
        "frequency": "daily",
        "units": "percent",
    },
    "us_payroll_employment": {
        "cdid": "PAYEMS",
        "title": "US total nonfarm payroll employment",
        "frequency": "monthly",
...
def fetch_fred_series(name: str, spec: dict, vintage: str) -> str:
    """Fetch a public FRED CSV series and retain observations from 2020."""
    query = urllib.parse.urlencode(
        {"id": spec["cdid"], "cosd": "2020-01-01"}
    )
    rows = csv.DictReader(io.StringIO(get_text(f"{FRED_CSV}?{query}")))
    observations = []
    for row in rows:
        raw_date = row.get("observation_date") or row.get("DATE")
        raw_value = row.get(spec["cdid"])
        if not raw_date or raw_value in (None, "", "."):
            continue
        ...
        observations.append({"period": period, "value": float(raw_value)})
    if not observations:
        raise SystemExit(f"{name}: no FRED observations parsed")
    ...
    snapshot = {
        ...
        "frequency": spec["frequency"],
        "units": spec["units"],
        "url": f"https://fred.stlouisfed.org/series/{spec['cdid']}",
    ...
    if out.exists():
        return "exists"
    snapshot["vintage"] = vintage
    snapshot["fetched_utc"] = datetime.now(timezone.utc).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
    series_dir.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(snapshot, indent=2) + "\n")
...
def rebuild_manifest() -> dict:
    ...
    manifest = {
        "_comment": (
            ...
            "copy for the static site. ONS and Bank of England data use the "
            "Open Government Licence v3.0; US series retain their publisher "
            "attribution through FRED."
        ),
        "licence": "Source-specific; see data/README.md",
        "series": entries,
    }
...
def main() -> int:
    ...
    all_series = {
        ...
        **{name: ("fred", spec) for name, spec in FRED_SERIES.items()},
    }
    ...
    for name, (source, spec) in wanted.items():
        try:
            if source == "ons":
                ...
                result = fetch_boe_series(name, spec, vintage)
            else:
                result = fetch_fred_series(name, spec, vintage)
